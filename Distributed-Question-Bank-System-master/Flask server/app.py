...
def Add():
    if request.method == 'POST':
        ...
        try:
            ...
            print(quest)
            if(len(quest)>1):
                checksimres = checkSim(json['Question'],quest)
                if(checksimres != []):
                    return {'Message': "Similar Question Already Exists in Database","ListQuestions": checksimres}, 206
...
@app.route('/AddUserChoice', methods=['POST'])
def AddUserChoice():
    if request.method == 'POST':
        json = request.json
        try:
...
def checkSim(quest,listques):
    ...
    # if(max(sims[query_quest_tf_idf])>=0.6):
    #     return True
    # else:
    #     return False
    x = sims[query_quest_tf_idf]
    print("i",x)
    sim_questions = []
    count = 0
    for i in x:
        if(i>=0.5):
            sim_questions.append(listques[count])
        count+=1
