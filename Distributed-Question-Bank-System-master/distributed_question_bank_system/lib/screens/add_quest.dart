...
//fill in the blanks
  ...
  addquest() async {
    try {
      ...
      //print(token);
      var urladd = Uri.parse(baseURL + '/Add');
      var urladduserchoice = Uri.parse(baseURL + '/AddUserChoice');
      var answer;
      var type;
      var keywords = [];
      ...
      if (response.statusCode == 200) {
        ...
        List listquest = jsonresp['ListQuestions'];
        List<Widget> qlist = [];
        var c = 0;
        listquest.forEach((i) {
          qlist.add(Text(
        ...
        return showDialog(
          ...
          builder: (ctx) => AlertDialog(
            backgroundColor: ThemeColors().secondaryColor,
            title: const Text(
                "Some of the Question Appeared to be similar to Your Question",
                style: TextStyle(
            ...
            content: Column(
              ...
              children: [
                const Text("Do You Still Want to Add this Question:",
            ...
            actions: <Widget>[
              TextButton(
                onPressed: () async {
                  var response = await http.post(urladduserchoice,
                      body: json.encode({
                        'token': token,
                        'Type': type,
                        'Question': quest.text.trim(),
